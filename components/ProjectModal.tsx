...
import { X, ChevronLeft, ChevronRight, Calendar, MapPin, FileText, Layers } from 'lucide-react';
        ...
        <div className="lg:w-3/5 bg-slate-900 relative flex flex-col h-[40vh] lg:h-auto group">
          <div className="flex-1 relative overflow-hidden bg-black">
              ...
              <>
                  <button 
                  onClick={prevImage}
                  className={`absolute ${isRTL ? 'right-4' : 'left-4'} top-1/2 -translate-y-1/2 bg-black/50 hover:bg-blue-600 text-white p-3 rounded-full transition-all z-20 backdrop-blur-sm border border-white/20 hover:border-blue-500 opacity-0 group-hover:opacity-100`}
                  >
                  <ChevronLeft size={24} className={isRTL ? 'rotate-180' : ''} />
                  </button>
                  <button 
                  onClick={nextImage}
                  className={`absolute ${isRTL ? 'left-4' : 'right-4'} top-1/2 -translate-y-1/2 bg-black/50 hover:bg-blue-600 text-white p-3 rounded-full transition-all z-20 backdrop-blur-sm border border-white/20 hover:border-blue-500 opacity-0 group-hover:opacity-100`}
                  >
        ...
        <div className="lg:w-2/5 p-8 flex flex-col overflow-y-auto bg-slate-50 custom-scrollbar border-l border-slate-200">
          <div className="flex justify-between items-start mb-6">
            <div className="flex flex-col">
                ...
                <h3 className="text-3xl font-bold text-slate-900 leading-tight">{project.title[lang]}</h3>
            </div>
            <button 
              onClick={onClose}
          ...
          <div className="mt-auto">
            ...
            <div className="flex flex-wrap gap-2">
              {['DIN VDE', 'ISO 9001', 'KNX Certified', 'CE'].map((tag, i) => (
                <span key={i} className="text-xs bg-white text-slate-600 px-3 py-1 rounded-full border border-slate-200 shadow-sm">
                  {tag}
                </span>
              ))}
            </div>
