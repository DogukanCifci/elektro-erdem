    ...
    <section id="testimonials" className="py-24 bg-slate-50 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-blue-600 font-semibold tracking-wider uppercase mb-2 text-sm">{t.testimonials.subtitle[lang]}</h2>
          <h3 className="text-3xl md:text-5xl font-bold text-slate-900">{t.testimonials.title[lang]}</h3>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((item, index) => (
            ...
            >
              <div className="absolute -top-4 left-8 rtl:left-auto rtl:right-8 text-6xl text-blue-500 opacity-20 font-serif">"</div>
              <p className="text-slate-600 mb-6 relative z-10 italic text-lg leading-relaxed">{item.comment[lang]}</p>
              <div className="flex items-center mt-auto border-t border-slate-100 pt-6">
                ...
                <div>
                  ...
                  <span className="text-sm text-slate-500">{item.role[lang]}</span>
                </div>
              </div>
            </motion.div>
          ))}
